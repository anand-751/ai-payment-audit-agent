...
def build_llm_prompt(metadata):
    ...
    return f"""
You are an enterprise treasury controller generating an executive audit narrative for a CFO dashboard.

The audit findings below are deterministic outputs generated by a validated payment compliance engine.

Your responsibility is to naturally summarize the operational and financial risk situation using the validated findings.

IMPORTANT:
- The narrative must sound executive and human-readable
- Reference important numerical findings naturally
- Avoid mechanically listing every metric
- Avoid robotic wording
- Avoid markdown
- Avoid bullet points
- Avoid conversational tone
- Do not hallucinate values or findings
- Keep the response concise and premium

The narrative should feel like a real AI-powered treasury intelligence system.

VALIDATED FINDINGS:

Integrity Classification: {metadata['integrity_label']}
*** Integrity Score: {metadata['integrity_score']}/100 ***
...
Approval Failures: {metadata['approval_failures']}
Vendor Issues: {metadata['vendor_issues']}
Amount Mismatches: {metadata['amount_mismatches']}

Discount Opportunities: {metadata['discount_opportunities']}
Missed Discounts: {metadata['discount_missed_count']}

Duplicate Invoice:
{metadata.get('duplicate_invoice_number', 'N/A')}

Generate ONLY the final treasury narrative.
""".strip()
...
def generate_ai_interpretation(metadata):
    ...
    try:
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "specializing in payment risk intelligence."
                    ),
                },
                {
                    "role": "user",
        ...
        return (
            "Critical payment integrity findings require "
            "treasury review before disbursement authorization."
        )
...
def validate_payment_batch(batch_id: str):
        ...
        if dup_info:
            metadata["duplicate_invoice_number"] = dup_info["invoice_number"]
