...
# ---------------------------------------------------
# PERSIST FULL AUDIT RESULT  (NEW)
# Stores the complete audit response (metadata + violations + cfo_summary)
# as JSON on the batch row, so the CFO can re-open it later WITHOUT
# re-running the audit (no LLM call, no status reset).
# ---------------------------------------------------

def save_audit_result(batch_id, audit_response):
    conn   = get_db_connection()
    cursor = conn.cursor()

    # Make sure the column exists (idempotent, mirrors csv_service pattern)
    cursor.execute("PRAGMA table_info(payment_batches)")
    columns = {row[1] for row in cursor.fetchall()}
    if "audit_json" not in columns:
        cursor.execute("ALTER TABLE payment_batches ADD COLUMN audit_json TEXT")

    cursor.execute(
        "UPDATE payment_batches SET audit_json = ? WHERE batch_id = ?",
        (json.dumps(audit_response, default=str), batch_id),
    )
